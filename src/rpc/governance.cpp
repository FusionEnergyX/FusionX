...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in fusion.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("fusion.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Fusion features */
    { "fusion",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "fusion",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "fusion",               "gobject",                &gobject,                true,  {} },
    { "fusion",               "voteraw",                &voteraw,                true,  {} },
